...
function addOp(str, isString) {
    ...
    if (str[0] == '<') {
        if (str[1] == '<') {
            return { '$lt' : str.substring(2) };
        } else {
            return { '$lte' : str.substring(1) };
        }
    } else if (str[0] == '>') {
        if (str[1] == '>') {
            return { '$gt' : str.substring(2) };
        } else {
            return { '$gte' : str.substring(1) };
        }
    } else if (str[0] == '!') {
        if (isString) {
            return { '$regex' : new RegExp('!('+str.substring(1)+')', 'i') };
        } else {
...
function filter(q, query, Model) {
    ...
    var path = Model.schema.path.bind(Model.schema);
